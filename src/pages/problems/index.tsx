...
import { getProblems, deleteProblemById } from 'apiSdk/problems';
import { ProblemInterface } from 'interfaces/problem';

type ColumnType = ColumnDef<ProblemInterface, unknown>;
...
export function ProblemListPage(props: ProblemListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProblemInterface>>(
    () => `/problems?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ProblemInterface) => {
    if (hasAccess('problem', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/problems/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/problems/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('problem', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/problems/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('problem', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="problem" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Problems
        </Text>
        {hasAccess('problem', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/problems/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
